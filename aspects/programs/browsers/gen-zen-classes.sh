...
UNI="Uni"
OB="$(readlink -f /usr/local/bin/obsidian)"
...
[ -x "$OB" ] || echo "warning: obsidian CLI not found at $OB, will fallback to markdown parse" >&2

# NB: every `obsidian` invocation MUST redirect `</dev/null`. The CLI drains stdin,
# so without this it eats the terminal (interactive runs hang to timeout -> empty) or
# a surrounding `while read` loop's input. Non-interactive runs mask the bug.
have_cli() { [ -x "$OB" ] && timeout 15 "$OB" files </dev/null >/dev/null 2>&1; }
USE_CLI=false
if have_cli; then USE_CLI=true; fi
echo "USE CLI: $USE_CLI"
...
list_notes() {
	local sub="$1"
	if [ "$USE_CLI" = true ]; then
		timeout 20 "$OB" files </dev/null 2>/dev/null
	else
...
while IFS= read -r tf; do
	...
	term="$(basename "$tf" .md)"
	year="$(prop year "$tf")"
	start="$(prop start_date "$tf")"
	end="$(prop end_date "$tf")"
	active="$(prop active "$tf")"
...
while IFS= read -r sf; do
	...
	[ -n "$term" ] || continue
	kind=""
	if [ "$term" = "$ACTIVE_TERM" ]; then
		kind="current"
	elif in_upcoming "$term"; then
		kind="upcoming"
	else continue; fi
	sid="$(prop subject_id "$sf")"
	[ -n "$sid" ] || continue
	...
	if [ -n "$cid" ]; then
		url="https://canvas.lms.unimelb.edu.au/courses/$cid"
