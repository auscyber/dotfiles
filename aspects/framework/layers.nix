...
in
{
  ...
  config.flake.scopeInputs =
    let
      ...
      classArgsFor =
        scope: mod:
        let
          wanted = builtins.attrNames (if builtins.isFunction mod then builtins.functionArgs mod else { });
          available = lib.filter (n: (sci.${scope} or { }) ? ${n}) wanted;
        in
        lib.genAttrs available (cls: map (e2: evalEntryIn scope e2) ((sci.${scope} or { }).${cls} or [ ]));
    ...
    lib.genAttrs (builtins.attrNames sci) (
      ...
      in
      {
        ...
        owner = lib.foldl' (
          acc: d: acc // lib.genAttrs (builtins.attrNames d.value) (_: d.entry.identity or "<anon>")
        ) { } decls;
        ...
        overrideBy = lib.foldl' (acc: e: acc // { ${e.identity or "<anon>"} = evalEntryIn scope e; }) { } (
  ...
  config.flake.layerPlan = builtins.seq _evalProbe (
    let
      ...
      allNames = lib.unique (lib.concatMap (h: builtins.attrNames (byHost.${h}.inputs or { })) hostNames);
      ...
      target =
        ...
        let
          p = flatten (rawTags n);
        in
        if p == [ ] then "root" else layerName p;
  ...
  config.flake.lib.layers = {
    ...
    shadowed =
      ...
      let
        ...
        claimsOf =
          name: lib.filter (s: s != null) (lib.mapAttrsToList (_: specs: specs.${name} or null) specsByLayer);
