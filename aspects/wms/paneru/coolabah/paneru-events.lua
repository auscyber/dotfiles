-- paneru window-manager coolabah provider (paneru's side).
--
-- The coolabah twin of ../sketchybar/paneru-events.lua, and it exists as a
...
-- paneru's event loop. coolabah has no equivalent module to load: its Lua API
-- is a Rust crate whose loadable `module`-feature cdylib is not something the
-- coolabah package builds, so nothing in paneru's interpreter can speak to the
-- daemon.
--
-- So this side does not draw. It pokes: each paneru event becomes one
-- `coolabah --trigger`, and coolabah's own config (../coolabah/wm.lua, which is
-- ../sketchybar/wm.lua — the same file, since coolabah answers to `sbar`)
...
-- interpreter. A failed trigger is logged and swallowed — coolabah not being up
...
local function trigger(event)
	local ok, err = pcall(paneru.exec, COOLABAH, { "--trigger", event })
	if not ok then
		paneru.log("coolabah trigger " .. event .. " failed: " .. tostring(err))
	end
end
...
-- First paint is coolabah's, not ours: this file runs at paneru's script-load
...
-- list is up — by then the daemon can answer coolabah's query, so announcing is
-- enough.
