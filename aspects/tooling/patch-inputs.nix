...
let
  ...
  generatedPatchedInputs =
    let
      ...
      entry =
        ...
        let
          ...
          # Recorded inline so flake.nix does not have to read
          # ./patches/hashes.json at all; `null` means "no recorded hash",
          # i.e. build locally and unsubstitutably.
          hashLine =
            "  ${name}.hash = ${if v.hash == null then "null" else "\"${v.hash}\""};";
        in
        "${patchLine}\n${hashLine}";
    ...
    ''
      ...
      # flake-parts module system: which inputs are patched, with which
      # patches, and the SRI hash of each resulting tree. Recording the hash
      # here means flake.nix never has to read ./patches/hashes.json.
      #
      # Everything else is still derived purely at eval time: `src` from the
      # flake input, and the auto-unified set from ./flake.lock.
      #
      # `hash = null` means no recorded hash. `PATCH_HASHES=ignore` still
      # overrides every hash here, which is the way out if one goes stale.
      #
      # `checks.patched-inputs-generated-current` fails if this drifts from the
      # aspects' own declarations. Refresh after `nix run .#update-patch-hashes`.
      {
      ${body}
      }
    '';
