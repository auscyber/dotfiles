...
(local buffer_not_empty (fn [] (if (~= 1 (vim.fn.empty (vim.fn.expand "%:t")))
                                true
                                false)))
...
(tset gls.left 1 {
                  :FirstElement {
                                 :provider (fn [] "  ")
                                 :highlight  [color.light_red]}})


(tset gls.left 2 {
                  :ViMode {
                           :provider (fn []
                                       (local mode_color {:n color.white :i  colors.cyan :v colors.blue
                                                                "" colors.blue :V colors.blue
                                                                :c color.purple :no colors.red :s colors.orange
...
(tset gls.right 1 {
                   :LspStatus {
                               :condition (fn [] (vim.lsp.buf_is_attached 0))
