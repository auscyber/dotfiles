   ...
   "
               (let [a (require :aniseed.core)
                 ...
                 (for [i 1 (a.count args) 2]
                   (let [name (. args i)
                         block (. args (+ i 1))]
                     (a.assoc block 1 name) ; (when (. block :mod) ;   (a.assoc block :config `(fn [] ;                               (require ,(.. :plugins. (tostring (. block :mod))))
                     ;                               (,(-?> block (. :config))))))
                     ;            (a.assoc block :mod)
                     (when (. block :config)
                 ...
                 (let [use-sym (gensym)]
                   `(let [lazy# (require :lazy)]
                      (lazy#.setup {:spec [,(unpack (icollect [_# v# (ipairs use-statements)]
                                                      v#))]})))))}
