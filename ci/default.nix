    ...
    { pkgs, system, ... }:
    {
      ...
      checks =
        ...
        let
          ...
          systemBuilds = lib.concatMapAttrs (name: v: {
            "check-system-builds-${name}" = v.config.system.build.toplevel;
          ...
          homeBuilds = lib.concatMapAttrs (name: v: {
            "check-home-builds-${name}" = v.config.home.activationPackage;
          }) homes;
