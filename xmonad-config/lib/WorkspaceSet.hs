...
-- | A set of workspaces
data WorkspaceSet = WorkspaceSet
    { _workspaceSetName :: WorkspaceSetId
...
data WorkspaceState = WorkspaceState
    { _currentWorkspaceSet :: WorkspaceSet
  ...
  where
    f modifiedTags layout winset =
        let tags = map W.tag $ W.workspaces winset
         ...
         in foldr
                (\x acc -> if x `elem` tags then acc else over W.hiddenL (createWorkspace x :) acc)
                winset
                modifiedTags

-- | Given a 'WorkspaceSetId' and a 'WorkspaceId' within that 'WorkspaceSet', show the true 'WorkspaceId'
fixTag ::
    -- | 'WorkspaceSet' that it is part of
    WorkspaceSetId ->
    -- | 'WorkspaceId' to fix
    WorkspaceId ->
    -- | Fixed 'WorkspaceId'
...
filterOutInvalidWSet pp = do
    ...
    pure $
        pp
            { ppSort = fmap (. {-over (traverse . SL.tagLens) (cleanWS right)  .-} f invalid) (ppSort pp)
